...
parser.add_argument('-t', '--title', help='Title of the ds9 display to connect to. Default is deimos_ccd_autodisplay', default=False)
...
if args.title:
   ...
   DS9_TITLE = 'deimos_ccd_autodisplay'
...
def wait_for_image(ds9_disp, last_file):
   ...
   if ( DEBUG_MODE == True ):
      ...
      print('current_orient_value = ', current_orient_value)
      print('current_scale_mode = ', current_scale_mode)
      print('current_regions_shape = ', current_regions_shape)
      print('current_file = ', current_file)

   return current_zoom, current_pan_mode, current_orient_value, \
      current_scale_mode, current_regions_shape, current_file


def display_current_file(ds9_disp, zoom, pan_mode, orient_value, scale_mode, regions_shape, filename):
   """ 
   ...
   if ( filename is None ):
      ...
      ds9_disp.openMosaic(filename, 1)
      ds9_disp.xpaset('zoom to ' + zoom)
      ds9_disp.xpaset('pan ' + pan_mode)
      ds9_disp.xpaset('orient ' + orient_value)
...
def startup():
   ...
   ds9_disp = ds9_ccd.ds9(DS9_TITLE)
...
file_exists = display_current_file(ds9_display, ZOOM_INI, PAN_INI, ORIENT_INI, SCALE_MODE_INI, REGIONS_SHAPE_INI, initial_file)

while True:

   # Loop for next image
   
   last_file, last_orient = path_to_file()
   zoom, pan_mode, orient_value, scale_mode, regions_shape, filename = wait_for_image(ds9_display, last_file)

   # Display current file 
   got_a_file = display_current_file(ds9_display, zoom, pan_mode, orient_value, scale_mode, regions_shape, filename)
